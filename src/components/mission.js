    ...
    <section ref={root} className="relative w-full overflow-hidden">
      ...
      <div className="relative z-10 mx-auto grid min-h-[480px] max-w-[1700px] grid-cols-1 px-5 py-12 sm:min-h-[560px] sm:px-8 sm:py-16 md:px-10 md:py-20 lg:min-h-[700px] lg:grid-cols-[minmax(0,1fr)_minmax(0,1.35fr)_minmax(0,0.5fr)] lg:py-0">
        ...
        <div className="relative z-20 flex flex-col justify-center py-4 sm:py-6 lg:py-24">
          ...
          {/* Heading - using SplitReveal for the same style as AboutStats */}
          <SplitReveal
            text="Driven by Purpose, Built on Promise"
            className="text-2xl font-bold uppercase leading-[1.2] tracking-tight text-black sm:text-3xl md:text-4xl xl:text-[2.75rem]"
          />

          {/* Divider line - matching AboutStats */}
          <motion.div
            ref={lineRef}
            initial={{ scaleX: 0 }}
          ...
          {/* Description - using WordReveal for the same style as AboutStats */}
          <WordReveal
            text={DESCRIPTION}
            delay={0.1}
            className="mt-5 max-w-full text-sm leading-[1.75] text-black/80 sm:mt-6 sm:max-w-md sm:text-base sm:leading-[1.85]"
          />
