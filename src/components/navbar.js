...
export default function Navbar() {
  ...
  return (
    ...
    >
      <nav
        className={`relative flex items-center justify-between gap-6 border-b border-[#1B3B8C]/15 px-6 transition-[padding] duration-300 sm:px-10 lg:justify-center lg:gap-10 ${
          scrolled ? "py-3" : "py-5"
        }`}
      >
        {/* LEFT — before logo */}
        <ul className="hidden items-center gap-10 lg:flex">
          {navLinks.map((link) => (
            <li key={link.label}>
              <Link
                href={link.href}
                className="whitespace-nowrap text-[15px] font-bold tracking-wide text-neutral-900 transition-colors hover:text-[#F0722F]"
              >
                {link.label}
              </Link>
            </li>
          ...
          {dropdownLinks.map((link) => (
            ...
            >
              <button
                type="button"
                className="flex items-center gap-1.5 text-[15px] font-bold tracking-wide text-neutral-900 transition-colors hover:text-[#F0722F]"
              >
                {link.label}
                <svg
        ...
        <Link href="/" className="flex flex-shrink-0 flex-col items-center" aria-label="PKR Estates Home">
          <Image
            ...
            className={`h-auto object-contain transition-all duration-300 ${
              scrolled ? "w-[150px] sm:w-[180px] lg:w-[130px]" : "w-[180px] sm:w-[220px] lg:w-[160px]"
            }`}
          />
        </Link>
        ...
        <button
          ...
          className="rounded-lg p-2 text-neutral-700 hover:bg-neutral-100 focus:outline-none lg:hidden"
      ...
      {mobileMenuOpen && (
        <div className="max-h-[calc(100svh-80px)] overflow-y-auto border-b border-neutral-200 bg-white px-6 py-4 shadow-lg lg:hidden">
